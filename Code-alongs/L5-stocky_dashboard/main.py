...
app.layout = html.Main(
    [
        ...
        dcc.Slider(id = "time-slider", min = 0, max = 6, marks = slider_marks, value = 2, step = None) #  step = none: diskreta värden, går inte att gå emellan
    ]
)

@app.callback(
    Output("stock-graph", "figure"), 
    Input("stockpicker-dropdown", "value"), # första
    Input("ohlc-radio", "value"),
    Input("time-slider", "value")
)
def update_graph(stock, ohlc, time_index): # första parameterna korrensponderar till första inputen, andra till andra inputen etc 
